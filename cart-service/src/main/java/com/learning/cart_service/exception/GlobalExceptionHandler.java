...
//    }
    ...
    public ResponseEntity<ApiResponse<Void>> handleProductServiceUnavailable(ProductServiceUnavailableException exception){
        log.error("Downstream Catalog Service unavailable: {}",exception.getMessage());
        return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE).body(ApiResponse.error(HttpStatus.SERVICE_UNAVAILABLE.value(), exception.getMessage(),null));
    }
