...
//    }
    ...
    // Layer-2 catch for File 1's composite unique constraint — the RARE
    // race-condition case that slips past createReview's existsBy() check.
    // Deliberately returns the SAME user-facing message as the
    // DuplicateResourceException path above — the caller shouldn't be able
    // to tell (or need to care) which layer caught it.
    @ExceptionHandler(org.springframework.dao.DataIntegrityViolationException.class)
    public ResponseEntity<ApiResponse<Void>> handleDataIntegrityViolation(
            org.springframework.dao.DataIntegrityViolationException ex) {
        log.warn("Data integrity violation: {}", ex.getMessage());
        return ResponseEntity.status(HttpStatus.CONFLICT).body(
                ApiResponse.error(HttpStatus.CONFLICT.value(), "You have already reviewed this product", null));
    }
